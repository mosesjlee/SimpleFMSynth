...
void SimpleEq::fillInputBuffer(float * buf, int numSamples, int numChannels){
...
float * SimpleEq::getInputBuffer(){
    return inputBuf;
}

void SimpleEq::fillOutputBuffer(float * buf, int numSamples, int numChannels){
    for (int i = 0; i < numSamples; i++) {
        for (int j = 0; j < 1; j++){
...
void SimpleEq::processNextSamples(){
    for (int i = 0; i < MAX_SAMPLES; i++) {
